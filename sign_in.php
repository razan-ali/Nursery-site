...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($stmt = $conn->prepare($sql)) {
        ...
        if ($result->num_rows > 0) {
            ...
            // Directly match the password (not recommended in production)
            if ($password == $row['password']) {
                // Start a session and store parent data for future use
                session_start();
                $_SESSION['parent_id'] = $row['id'];
                $_SESSION['parent_name'] = $row['name'];
                // Redirect to parent's dashboard
        ...
        <section class="auth-container">
            <div class="auth-box">
                ...
                <form action="sign_in.php" method="POST">
                    ...
                    <p>ليس لديك حساب؟ <a href="sign_up.php">إنشاء حساب</a></p>
                </form>
            </div>
        </section>
    </main>
